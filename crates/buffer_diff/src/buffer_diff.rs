...
fn compute_hunks(
    ...
    if let Some((diff_base, diff_base_rope)) = diff_base {
        ...
        // A common case in LEAD is that the empty buffer is represented as just a newline,
...
mod tests {
    ...
    async fn test_stage_all_with_stale_buffer(cx: &mut TestAppContext) {
        // Regression test for LEAD-5R2: when the buffer is edited after the diff is
