...
//! pipeline. LEAD opts into that pipeline during rendering, then keeps
...
//! - [`postprocess`] the SVG to add LEAD theme and accent styling.
//!
//! LEAD's postprocessing is split up into stages. We parse the generated SVG
...
//! `player_colors` in the LEAD theme.
//!
...
//!    `LEAD-accent-0`) to specific elements, based on the diagram type and
//!    node.
...
/// from LEAD's UI palette) round-trip without silently losing their alpha.
