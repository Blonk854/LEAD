    ...
    v_flex()
        ...
        .child(
            div()
                ...
                .tooltip(Tooltip::text("Click to open by running LEAD CLI"))
...
async fn open_zed_source_location(
    ...
    let output = new_command("LEAD")
        .arg(&path_arg)
        .output()
        .await
        .with_context(|| format!("running LEAD to open {path_arg} failed"))?;

    if !output.status.success() {
        Err(anyhow!(
            "running LEAD to open {path_arg} failed with stderr: {}",
