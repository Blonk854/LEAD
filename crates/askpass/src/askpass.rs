...
/// On Windows, this is set to the CLI variant of LEAD
...
impl PasswordProxy {
    pub async fn new(
        ...
        let temp_dir = tempfile::Builder::new().prefix("LEAD-askpass").tempdir()?;
...
/// The main function for when LEAD is running in netcat mode for use in askpass.
/// Called from both the remote server binary and the LEAD binary in their respective main functions.
