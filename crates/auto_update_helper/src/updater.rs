...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("LEAD.exe"), p("old\\LEAD.exe")),
        Job::mkdir(p("old\\bin")),
        Job::move_file(p("bin\\LEAD.exe"), p("old\\bin\\LEAD.exe")),
        Job::move_file(p("bin\\LEAD"), p("old\\bin\\LEAD")),
        //
        ...
        // Copy new files
        Job::move_file(p("install\\LEAD.exe"), p("LEAD.exe")),
        Job::move_file(p("install\\bin\\LEAD.exe"), p("bin\\LEAD.exe")),
        Job::move_file(p("install\\bin\\LEAD"), p("bin\\LEAD")),
        //
...
fn release_file_handles(app_dir: &Path) -> Result<()> {
    ...
    let files_to_release = [
        app_dir.join("LEAD.exe"),
        app_dir.join("bin\\LEAD.exe"),
        app_dir.join("bin\\LEAD"),
...
pub(crate) fn perform_update(app_dir: &Path, hwnd: Option<isize>, launch: bool) -> Result<()> {
    ...
    if launch {
        ...
        let _ = std::process::Command::new(app_dir.join("LEAD.exe")).spawn();
    }
