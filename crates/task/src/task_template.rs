...
/// A template definition of a LEAD task to run.
...
/// contains all relevant LEAD state in task variables.
...
fn substitute_all_template_variables_in_str<A: AsRef<str>>(
    ...
    let substituted_string = shellexpand::env_with_context(template_str, |var| {
        ...
        if let Some(name) = task_variables.get(variable_name) {
            ...
            // Unknown LEAD variable - use default if available
            if !default.is_empty() {
...
mod tests {
    ...
    fn test_template_variables_resolution() {
        ...
        for i in 0..all_variables.len() {
            ...
            assert!(
                ...
                "If any of the LEAD task variables is not substituted, the task should not be resolved, but got some resolution without the variable {removed_variable:?} (index {i})"
            );
        }
    }
    ...
    fn test_variable_default_values() {
        ...
        // Test 3: Missing LEAD variable without default should fail
        ...
        assert!(
            task_no_default
                .resolve_task(TEST_ID_BASE, &TaskContext::default())
                .is_none(),
            "Should fail when LEAD variable has no default and doesn't exist"
        );
    }
