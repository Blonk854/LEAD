...
#[command(
    ...
    before_help = "The LEAD CLI binary.
This CLI is a separate binary that invokes LEAD.
...
Examples:
    ...
    `LEAD path-to-your-project`
          Open your project in LEAD
    `LEAD -n path-to-file `
          Open file/folder in a new window",
    after_help = "To read from stdin, append '-', e.g. 'ps axf | LEAD -'"
)]
struct Args {
    ...
    #[cfg_attr(target_os = "macos", doc = "`~/Library/Application Support/LEAD`.")]
    #[cfg_attr(target_os = "windows", doc = "`%LOCALAPPDATA%\\LEAD`.")]
    #[cfg_attr(
        not(any(target_os = "windows", target_os = "macos")),
        doc = "`$XDG_DATA_HOME/LEAD`."
    )]
    #[arg(long, value_name = "DIR")]
    user_data_dir: Option<String>,
    /// The paths to open in LEAD (space-separated).
    ///
    /// Use `path:line:column` syntax to open a file at the given line and column.
    paths_with_position: Vec<String>,
    /// Print LEAD's version and the app path.
    #[arg(short, long)]
    version: bool,
    /// Run LEAD in the foreground (useful for debugging)
    #[arg(long)]
    foreground: bool,
    /// Custom path to LEAD.app or the LEAD binary
    #[arg(long)]
    zed: Option<PathBuf>,
    /// Run LEAD in dev-server mode
    #[arg(long)]
    ...
    wsl: Option<String>,
    /// Not supported in LEAD CLI, only supported on LEAD binary
    ...
    diff: Vec<String>,
    /// Uninstall LEAD from user system
    #[cfg(all(
...
///
/// This method must return an absolute path, as many LEAD
...
fn run() -> Result<()> {
    ...
    if args.system_specs {
        let path = app.path();
        let msg = [
            "The `--system-specs` argument is not supported in the LEAD CLI, only on LEAD binary.",
    ...
    let env = {
        #[cfg(any(target_os = "linux", target_os = "freebsd"))]
        {
            ...
            // On Linux, the desktop entry uses `cli` to spawn `LEAD`.
    ...
    // for directory diffs. The CLI process may exit before LEAD has read these
...
/// behavior for `LEAD <path>`. Returns `None` if the prompt cannot be shown
...
fn prompt_open_behavior() -> Option<cli::CliBehaviorSetting> {
    ...
    let items = [
        format!(
            "Add to existing LEAD window ({})",
            blue.apply_to("LEAD --existing")
        ),
        format!("Open a new window ({})", blue.apply_to("LEAD --classic")),
    ];

    let prompt = format!(
        "Configure default behavior for {}\n{}",
        blue.apply_to("LEAD <path>"),
        console::style("You can change this later in LEAD settings"),
    );
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                // libexec is the standard, lib/LEAD is for Arch (and other non-libexec distros),
                // ./LEAD is for the target directory in development builds.
                let possible_locations = [
                    "../libexec/LEAD-editor",
                    "../lib/LEAD/LEAD-editor",
                    "./LEAD",
                ];
                possible_locations
                    .iter()
...
mod flatpak {
    ...
    pub fn try_restart_to_host() {
        if let Some(flatpak_dir) = get_flatpak_dir() {
            ...
            args.push("--env=ZED_UPDATE_EXPLANATION=Please use flatpak to update LEAD".into());
            args.push(
                format!(
            ...
            args.push(flatpak_dir.join("bin").join("LEAD").into());

            let mut is_app_location_set = false;
            for arg in &env::args_os().collect::<Vec<_>>()[1..] {
                args.push(arg.clone());
                is_app_location_set |= arg == "--LEAD";
            }

            if !is_app_location_set {
                args.push("--LEAD".into());
                args.push(flatpak_dir.join("libexec").join("LEAD-editor").into());
            }
    ...
    pub fn set_bin_if_no_escape(mut args: super::Args) -> super::Args {
        if env::var(NO_ESCAPE_ENV_NAME).is_ok()
            ...
            && args.zed.is_none()
        {
            args.zed = Some("/app/libexec/LEAD-editor".into());
            unsafe {
                env::set_var(
                    "ZED_UPDATE_EXPLANATION",
                    "Please use flatpak to update LEAD",
                )
            };
        }
        args
    ...
    impl Detect {
        pub fn detect(path: Option<&Path>) -> anyhow::Result<impl InstalledApp> {
            let path = if let Some(path) = path {
                ...
                let installed_app = format!("../{}.exe", paths::APP_NAME);
                let dev_app = format!("./{}.exe", paths::APP_NAME_LOWERCASE);
                let possible_locations = [
                    installed_app.as_str(),
                    "../lib/LEAD/LEAD-editor.exe",
                    dev_app.as_str(),
                ];
                possible_locations
                    .iter()
...
mod mac_os {
    ...
    impl InstalledApp for Bundle {
        fn zed_version_string(&self) -> String {
            format!("LEAD {} – {}", self.version(), self.path().display(),)
        }
            ...
            match self {
                Self::App { app_bundle, .. } => {
                    ...
                    let status = unsafe {
                        ...
                        // equivalent to: open LEAD-cli:... -a /Applications/LEAD\ Preview.app
                        let urls_to_open =
        ...
        fn run_foreground(
            ...
            let path = match self {
                Bundle::App { app_bundle, .. } => app_bundle.join("Contents/MacOS/LEAD"),
