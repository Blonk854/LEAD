...
/// same `RemoteConnectionIdentity`, matching how LEAD treats multiple
...
async fn test_closing_last_agent_panel_terminal_restores_empty_header(cx: &mut TestAppContext) {
    ...
    assert_eq!(
        visible_entries_as_strings(&sidebar, cx),
        vec!["v [my-project]", "  New LEAD Agent Thread"]
    );
    assert_project_header_has_threads(&sidebar, "my-project", true, cx);
...
// `[/cloud, /worktrees/LEAD/wt_a/LEAD]`, where:
//   - `/cloud` is a standalone git repo (main == folder).
//   - `/worktrees/LEAD/wt_a/LEAD` is a linked worktree of `/LEAD`.
//
// Once git scans complete the project group key is
// `[/cloud, /LEAD]` — the main paths of the two roots. A thread
// created in this workspace is written with
// `main=[/cloud, /LEAD], folder=[/cloud, /worktrees/LEAD/wt_a/LEAD]`
...
// `[/cloud, /worktrees/LEAD/wt_a/LEAD]` ≠ `[/cloud, /LEAD]`. The three
// lookups in `rebuild_contents` all miss:
//
//   1. `entries_for_main_worktree_path([/cloud, /LEAD])` — the
//      thread's stored main doesn't equal the group key.
//   2. `entries_for_path([/cloud, /LEAD])` — the thread's folder paths
...
//      thread's folder `/worktrees/LEAD/wt_a/LEAD` doesn't match.
//
...
async fn test_sidebar_keeps_multi_root_thread_with_stale_main_paths(cx: &mut TestAppContext) {
    ...
    // Separate /LEAD repo whose linked worktree will form the second
    // workspace root. /LEAD itself is NOT opened as a workspace root.
    fs.insert_tree(
        "/LEAD",
        serde_json::json!({
            ".git": {},
    ...
    .await;
    fs.insert_tree(
        "/worktrees/LEAD/wt_a/LEAD",
        serde_json::json!({
            ".git": "gitdir: /LEAD/.git/worktrees/wt_a",
            "src": {},
        }),
    )
    .await;
    fs.add_linked_worktree_for_repo(
        ...
        false,
        git::repository::Worktree {
            path: std::path::PathBuf::from("/worktrees/LEAD/wt_a/LEAD"),
    ...
    let project = project::Project::test(
        fs.clone(),
        ["/cloud".as_ref(), "/worktrees/LEAD/wt_a/LEAD".as_ref()],
        cx,
    )
    .await;
    project.update(cx, |p, cx| p.git_scans_complete(cx)).await;
    ...
    let expected_main_paths = PathList::new(&[PathBuf::from("/cloud"), PathBuf::from("/LEAD")]);
    assert_eq!(
        group_key.path_list(),
        &expected_main_paths,
        "expected the multi-root workspace's project group key to normalize to \
         [/cloud, /LEAD] (main of the standalone repo + main of the linked worktree)"
    );

    let folder_paths = PathList::new(&[
        PathBuf::from("/cloud"),
        PathBuf::from("/worktrees/LEAD/wt_a/LEAD"),
    ]);
    let workspace_root_paths = workspace.read_with(cx, |ws, cx| PathList::new(&ws.root_paths(cx)));
    assert_eq!(
        workspace_root_paths, folder_paths,
        "expected the workspace's root paths to equal [/cloud, /worktrees/LEAD/wt_a/LEAD]"
    );
...
async fn test_remote_archive_thread_with_active_connection(
    ...
    // linked worktree across two remote projects the way LEAD does in
