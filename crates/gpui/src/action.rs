...
///
/// The namespace argument `editor` can also be omitted, though it is required for LEAD actions.
#[macro_export]
macro_rules! actions {
...
/// specified. In LEAD these trait impls are used to load keymaps from JSON.
///
/// Multiple arguments separated by commas may be specified in `#[action(...)]`:
///
/// - `namespace = some_namespace` sets the namespace. In LEAD this is required.
///
...
///   can then still be used to refer to invoke this action. In LEAD, the keymap JSON schema will
///   accept these old names and provide warnings.
///
/// - `deprecated = "Message about why this action is deprecation"` specifies a deprecation message.
///   In LEAD, the keymap JSON schema will cause this to be displayed as a warning.
///
...
pub trait Action: Any + Send {
    ...
    /// invoke the action. In LEAD, the keymap JSON schema will accept these old names and provide
    /// warnings.
    ...
    /// Returns the deprecation message for this action, if any. In LEAD, the keymap JSON schema will
...
/// Error type for `Keystroke::parse`. This is used instead of `anyhow::Error` so that LEAD can use
