...
impl A11y {
    ...
    pub(crate) fn end_frame(&mut self) -> TreeUpdate {
        ...
        // LEAD currently doesn't set any a11y APIs on *any* UI elements, so a
        ...
        if tree_update.nodes.len() > 1 {
            log::warn!(
                "expected an empty a11y tree update (only the root node), but got {} nodes; LEAD has no accessible UI elements yet",
