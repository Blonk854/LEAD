...
use crate::{
    ...
    pages::{
        ...
        render_tool_permissions_setup_page, render_unleashed_setup_page,
    },
};
...
fn developer_page(cx: &App) -> SettingsPage {
    ...
    items.push(SettingsPageItem::SettingItem(SettingItem {
        ...
        description: "Collect timing data for foreground and background executor tasks so they can be inspected via `LEAD: open performance profiler`. May lead to increased memory usage.",
    ...
    fn general_settings_section(_cx: &App) -> Vec<SettingsPageItem> {
        vec![
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "How `LEAD <path>` opens directories when no flag is specified.",
                field: Box::new(SettingField {
    ...
    fn security_section() -> [SettingsPageItem; 2] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "When opening LEAD, avoid Restricted Mode by auto-trusting all projects, enabling use of all features without having to give permission to each new project.",
                field: Box::new(SettingField {
    ...
    fn workspace_restoration_section() -> [SettingsPageItem; 3] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "What to restore from the previous session when opening LEAD.",
                field: Box::new(SettingField {
    ...
    fn scoped_settings_section() -> [SettingsPageItem; 3] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "Which settings should be activated only in Preview build of LEAD.",
                field: Box::new(
                    SettingField {
    ...
    fn privacy_section() -> [SettingsPageItem; 3] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "Send anonymized usage data like what languages you're using LEAD with.",
                field: Box::new(SettingField {
        ...
        [
            ...
            SettingsPageItem::DynamicItem(DynamicItem {
                discriminant: SettingItem {
                    files: USER,
                    title: "Icon Theme",
                    description: "The custom set of icons LEAD will associate with files and directories.",
                    field: Box::new(SettingField {
    ...
    fn file_scan_section() -> [SettingsPageItem; 6] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "Files or globs of files that will be excluded by LEAD entirely. They will be skipped during file scans, file searches, and not be displayed in the project file tree. Takes precedence over \"File Scan Inclusions\"",
                field: Box::new(
                    SettingField {
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "Files or globs of files that will be included by LEAD, even when ignored by git. This is useful for files that are not tracked by git, but are still important to your project. Note that globs that are overly broad can slow down LEAD's file scanning. \"File Scan Exclusions\" takes precedence over these inclusions",
                field: Box::new(
                    SettingField {
    ...
    fn window_section() -> [SettingsPageItem; 3] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "(Linux only) whether LEAD or your compositor should draw window decorations.",
                field: Box::new(SettingField {
    ...
    fn general_section() -> [SettingsPageItem; 6] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "Whether breakpoints should be reused across LEAD sessions.",
                field: Box::new(SettingField {
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "Whether to log messages between active debug adapters and LEAD.",
                field: Box::new(SettingField {
        ...
        [
            ...
            SettingsPageItem::DynamicItem(DynamicItem {
                discriminant: SettingItem {
                    ...
                    description: "Disable all Git integration features in LEAD.",
                    field: Box::new(SettingField::<bool> {
...
fn ai_page(cx: &App) -> SettingsPage {
    fn general_section() -> [SettingsPageItem; 3] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "Disable AI",
                description: "Whether to disable all AI features in LEAD.",
                field: Box::new(SettingField {
    ...
    fn agent_configuration_section(_cx: &App) -> Box<[SettingsPageItem]> {
        ...
        items.extend([
            SettingsPageItem::SectionHeader("Unleashed"),
            SettingsPageItem::SettingItem(SettingItem {
                title: "Enable Unleashed",
                description: "Master switch for whole-PC agent tools (run code, process control, HTTP requests, and computer use). Keep the Unleashed agent profile selected to expose those tools.",
                field: Box::new(SettingField {
                    json_path: Some("agent.full_access.enabled"),
                    pick: |settings_content| {
                        settings_content
                            .agent
                            .as_ref()?
            ...
            SettingsPageItem::SubPageLink(SubPageLink {
                title: "Unleashed Setup".into(),
                r#type: Default::default(),
                json_path: Some("agent.full_access"),
                description: Some(
                    "Configure trusted and blocked roots, plus permission rules for Unleashed tools."
                        .into(),
                ),
                in_json: true,
                files: USER,
                render: render_unleashed_setup_page,
            }),
        ]);
...
fn network_page() -> SettingsPage {
    fn network_section() -> [SettingsPageItem; 3] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "Server URL",
                description: "The URL of the LEAD server to connect to.",
                field: Box::new(SettingField {
...
fn language_settings_data() -> Box<[SettingsPageItem]> {
    ...
    fn autoclose_section() -> [SettingsPageItem; 5] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "Use Autoclose",
                description: "Whether to automatically type closing characters for you. For example, when you type '(', LEAD will automatically add a closing ')' at the correct position.",
                field: Box::new(SettingField {
            ...
            SettingsPageItem::SettingItem(SettingItem {
                ...
                description: "Whether to automatically surround text with characters for you. For example, when you select text and type '(', LEAD will automatically surround text with ().",
                field: Box::new(SettingField {
    ...
    fn tasks_section() -> [SettingsPageItem; 4] {
        [
            ...
            SettingsPageItem::SettingItem(SettingItem {
                title: "Prefer LSP",
                description: "Use LSP tasks over LEAD language extension tasks.",
                field: Box::new(SettingField {
...
fn edit_prediction_language_settings_section() -> [SettingsPageItem; 5] {
    [
        ...
        SettingsPageItem::SubPageLink(SubPageLink {
            ...
            description: Some("Set up different edit prediction providers in complement to LEAD's built-in Zeta model.".into()),
            in_json: false,
            files: USER,
        ...
        SettingsPageItem::SettingItem(SettingItem {
            ...
            description: "Controls whether LEAD may collect training data when using LEAD's Edit Predictions. Data is only collected for files in projects detected as open source. The default value uses the preference previously set via the status-bar toggle, or false if no preference has been stored.",
            field: Box::new(SettingField {
