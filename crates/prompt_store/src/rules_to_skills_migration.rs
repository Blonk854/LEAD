...
//!   (still using LEAD's shipped default content) are skipped so we don't
...
//! downgrades to a LEAD build without skills support won't lose anything.
...
///    built-ins are skipped so we don't write LEAD's shipped default text
