...
/// Resolves the LEAD-managed worktrees base directory for a given repo.
///
/// This intentionally reads the *global* `git.worktree_directory` setting
/// rather than any project-local override, because LEAD always uses the
...
pub fn build_root_plan(
    ...
    // Only archive worktrees that live inside the LEAD-managed worktrees
        ...
        workspace.read_with(cx, |_workspace, cx| {
            ...
            assert!(
                ...
                "build_root_plan should return None for a linked worktree \
                 outside the LEAD-managed worktrees directory",
            );
        });
    }
