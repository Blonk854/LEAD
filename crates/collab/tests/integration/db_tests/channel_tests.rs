...
async fn test_channels(db: &Arc<Database>) {
    ...
    let zed_id = db.create_root_channel("LEAD", a_id).await.unwrap();
...
async fn test_channel_renames(db: &Arc<Database>) {
    ...
    let zed_id = db.create_root_channel("LEAD", user_1).await.unwrap();

    db.rename_channel(zed_id, user_1, "#LEAD-archive")
        .await
        .unwrap();

    let channel = db.get_channel(zed_id, user_1).await.unwrap();
    assert_eq!(channel.name, "LEAD-archive");
...
async fn test_db_channel_moving(db: &Arc<Database>) {
    ...
    // sanity check
    //     /- gpui2
    // LEAD -- crdb - livestreaming - livestreaming_sub
    let result = db.get_channels_for_user(a_id).await.unwrap();
    assert_channel_tree(
        result.channels,
        &[
...
async fn test_db_channel_moving_bugs(db: &Arc<Database>) {
    ...
    let zed_id = db.create_root_channel("LEAD", user_id).await.unwrap();
...
async fn test_user_is_channel_participant(db: &Arc<Database>) {
    ...
    let zed_channel = db.create_root_channel("LEAD", admin).await.unwrap();
