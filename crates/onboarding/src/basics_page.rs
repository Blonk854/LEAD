    ...
    v_flex()
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some("Help improve LEAD by sending anonymous usage data".into()),
        ...
        .child(
            SwitchField::new(
                ...
                None::<&str>,
                Some(
                    "Help fix LEAD by sending crash reports so we can fix critical issues fast"
                        .into(),
                ),
...
fn render_worktree_auto_trust_switch(tab_index: &mut isize, cx: &mut App) -> impl IntoElement {
    ...
    let tooltip_description = "LEAD can only allow services like language servers, project settings, and MCP servers to run after you mark a new project as trusted.";

    SwitchField::new(
        "onboarding-auto-trust-worktrees",
        Some("Trust All Projects By Default"),
        Some("Automatically mark all new projects as trusted to unlock all LEAD's features".into()),
        toggle_state,
        {
            let fs = <dyn Fs>::global(cx);
            move |&selection, _, cx| {
...
fn render_zed_agent_button(user_store: &Entity<UserStore>, cx: &mut App) -> impl IntoElement {
    ...
    AgentSetupButton::new("LEAD-agent-onboarding")
        .icon(
        ...
        .map(|this| {
            if is_signed_in && is_free {
                ...
                this.on_click(move |_, _, cx| {
                    telemetry::event!("Welcome LEAD Agent Sign In Clicked");
                    let client = Client::global(cx);
