//! A module, responsible for managing the trust logic in LEAD.
//!
...
//!
//! LEAD does not consider invisible, `worktree.is_visible() == false` worktrees in LEAD, as those are programmatically created inside LEAD for internal needs, e.g. a tmp dir for `keymap_editor.rs` needs.
//!
//!
//! Path rust hierarchy.
//!
//! LEAD has multiple layers of trust, based on the requests and [`PathTrust`] enum variants.
//! From the least to the most trusted level:
//!
//! * "single file worktree"
//!
//! After opening an empty LEAD it's possible to open just a file, same as after opening a directory in LEAD it's possible to open a file outside of this directory.
//! Usual scenario for both cases is opening LEAD's settings.json file via `LEAD: open settings file` command: that starts a language server for a new file open, which originates from a newly created, single file worktree.
//!
//! Spawning a language server is potentially dangerous, and LEAD needs to restrict that by default.
//! Each single file worktree requires a separate trust permission, unless a more global level is trusted.
//!
//! * "directory worktree"
//!
//! If a directory is open in LEAD, it's a full worktree which may spawn multiple language servers associated with it.
...
//! * "path override"
//!
//! To ease trusting multiple directory worktrees at once, it's possible to trust a parent directory of a certain directory worktree opened in LEAD.
...
impl TrustedWorktreesStore {
    ...
    /// Requires LEAD's restart to take proper effect.
    ...
    pub fn can_trust(
        ...
        // LEAD opened an "internal" directory: e.g. a tmp dir for `keymap_editor.rs` needs.
