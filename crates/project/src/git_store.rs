...
/// Represents the handler of a git cat-file --batch process within LEAD
...
impl RepositorySnapshot {
    ...
    /// created from.
    ///
    /// For example, if you had both `~/code/LEAD` and `~/code/worktrees/LEAD-2`,
    /// then `~/code/LEAD` is the main worktree and `~/code/worktrees/LEAD-2` is a linked worktree.
    ///
    ...
    pub fn access(&mut self, _cx: &App) -> oneshot::Receiver<GitAccess> {
        self.send_job("access", None, move |git_repo, _cx| async move {
            match git_repo {
                ...
                // that's running the LEAD remote may not own the `.git/`
...
/// - Otherwise (e.g. `LEAD.git` for a bare clone), `common_dir` itself is
