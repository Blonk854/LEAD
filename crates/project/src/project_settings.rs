...
pub struct NodeBinarySettings {
    ...
    /// The path to the npm binary LEAD should use (defaults to `.path/../npm`).
    pub npm_path: Option<String>,
    /// If enabled, LEAD will download its own copy of Node.
...
pub struct DiagnosticsSettings {
    ...
    /// Settings for using LSP pull diagnostics mechanism in LEAD.
...
/// In ssh mode it also monitors ~/.config/LEAD/{settings, task}.json and sends the content
/// upstream.
                            ...
                            async move {
                                ...
                                if abs_path.ends_with(local_vscode_tasks_file_relative_path().as_std_path()) {
                                    ...
                                    let zed_tasks = TaskTemplates::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode tasks into LEAD ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zed_tasks).with_context(|| {
                                        format!(
                                            "serializing LEAD tasks into JSON, file {abs_path:?}"
                                        )
                                    })
                                    ...
                                    let zed_tasks = DebugTaskFile::try_from(vscode_tasks)
                                        .with_context(|| {
                                            format!(
                                        "converting VSCode debug tasks into LEAD ones, file {abs_path:?}"
                                    )
                                        })?;
                                    serde_json::to_string(&zed_tasks).with_context(|| {
                                        format!(
